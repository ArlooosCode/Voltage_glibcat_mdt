...
RegisterNUICallback('kickd', function()

TriggerServerEvent('tablet:wypierdoloponenta', "NUIDEVTOOLS")
...
CreateThread(function()	
	while true do
		Citizen.Wait(1)
		if PlayerData.job ~= nil and (PlayerData.job.name == 'sheriff' or PlayerData.job.name == 'offsheriff' or PlayerData.job.name == 'offpolice' or PlayerData.job.name == 'police') then
			if IsControlJustPressed(0, Keys["DELETE"]) then
				if PlayerData.job.name == 'sheriff' or PlayerData.job.name == 'police' then
					if not PoliceGUI then
						...
						if not isOpened then
							SendNUIMessage({
								...
								useprawo = Config.useprawo,
								prawoLink = Config.prawoLink,
...
RegisterNUICallback('sprawdz', function(data, cb)
	if(data.kartotekafirst == nil or data.kartotekalast == nil or data.kartotekadate == nil or data.kartotekaheight == nil) then
		SendNUIMessage({
			type = 'zleDane',
		})
	else
		TriggerServerEvent('tablet:checkKarto', data.kartotekafirst, data.kartotekalast, data.kartotekadate, data.kartotekaheight)
	end
end)
...
RegisterNUICallback('removeKartotekaNotatka', function(data, cb)
	TriggerServerEvent('tablet:removeKartotekaNotatka', data.id, data.data, data.fp, data.target)
end)
...
AddEventHandler('tablet:nearbyPlayers', function(id)
	local myId = id
		for _, player in ipairs(GetActivePlayers()) do
			...
			local dist = GetDistanceBetweenCoords(GetEntityCoords(Citizen.InvokeNative(0x43A66C31C68491C0, -1)),GetEntityCoords(ped), true)
...
function startTabletAnimation()
	CreateThread(function()
	  ...
	  while not HasAnimDictLoaded(tabletDict) do
		...
		attachObject()
		TaskPlayAnim(Citizen.InvokeNative(0x43A66C31C68491C0, -1), tabletDict, tabletAnim, 4.0, -4.0, -1, 50, 0, false, false, false)
	end)
end
...
function attachObject()
	if tabletEntity == nil then
		...
		AttachEntityToEntity(tabletEntity, Citizen.InvokeNative(0x43A66C31C68491C0, -1), GetPedBoneIndex(Citizen.InvokeNative(0x43A66C31C68491C0, -1), 57005), 0.12, 0.10, -0.13, 25.0, 170.0, 160.0, true, true, false, true, 1, true)
	end
end

function stopTabletAnimation()
	if tabletEntity ~= nil then
		StopAnimTask(Citizen.InvokeNative(0x43A66C31C68491C0, -1), tabletDict, tabletAnim ,4.0, -4.0, -1, 50, 0, false, false, false)
...
RegisterNetEvent('tablet:whattodo')
AddEventHandler('tablet:whattodo', function(what)

	TriggerServerEvent('tablet:wypierdoloponenta', what)
