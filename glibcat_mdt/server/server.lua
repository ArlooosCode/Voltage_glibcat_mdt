...
AddEventHandler('tablet:wystawMandat', function(target, fp, reason, fine, name, odznaka)
	...
	local praca = sourceXPlayer.getJob().name
	if praca == 'police' or praca == 'sheriff' then
	targetXPlayer.removeAccountMoney('bank', mandat)
	targetXPlayer.showNotification('~b~Dostałeś fakturę na ' ..mandat..'$ od ' .._source)
	...
	TriggerClientEvent('tablet:whattodo', source, "MANDAT")
	--TriggerEvent('DAWIJMDAWIJD8945J09J9GEJG90JE5490', source, '','', 'MANDAT')
end
end)
...
AddEventHandler('tablet:wyslijDoPaki', function(target, fp, reason, fine, years, name, odznaka)
	...
	if praca == 'police' or praca == 'sheriff' then
	targetXPlayer.removeAccountMoney('bank', mandat)
	targetXPlayer.showNotification('~b~Dostałeś fakturę na ' ..mandat..'$ i '..years..' miesięcy więzienia od ' .._source)
	sourceXPlayer.addAccountMoney('bank', math.floor(mandat * Config.officerReward))
	...
	TriggerEvent("esx_jtestailer:sendToJailPanelMust", target, years * 60, reason, mandat, fp)
	
	if Config.useWebhooks then
		jailWebhook(fp, fine, name, reason, years, _source)
	end
else
	TriggerClientEvent('tablet:whattodo', source, "JAIL")
	--TriggerEvent('DAWIJMDAWIJD8945J09J9GEJG90JE5490', source, '','', 'JAIL')
end
...
AddEventHandler('tablet:checkKarto', function(firstname, lastname, dateofbirth, height)
	local _source = source
	
	local basic = MySQL.Sync.fetchAll('SELECT * FROM users WHERE firstname = @firstname AND lastname = @lastname AND dateofbirth = @dateofbirth AND height = @height', {
		['@firstname'] = firstname,
		['@lastname'] = lastname,
	...
	if basic[1] then
		...
		local kartoteka = MySQL.Sync.fetchAll('SELECT * FROM user_kartoteka WHERE identifier = @identifier AND charid = @charid', {
			['@identifier'] = id,
			['@charid'] = charid[1].charid
		})
		
		local poszukiwania = MySQL.Sync.fetchAll('SELECT * FROM user_poszukiwania WHERE identifier = @identifier AND charid = @charid', {
			['@identifier'] = id,
			['@charid'] = charid[1].charid
		})
		
		local licenses = MySQL.Sync.fetchAll('SELECT * FROM user_licenses WHERE owner = @identifier AND charid = @charid', {
			['@identifier'] = id,
			['@charid'] = charid[1].charid
		})
		
		local cars = MySQL.Sync.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @identifier AND charid = @charid', {
			['@identifier'] = id,
			['@charid'] = charid[1].charid
		})
		
		local notatki = MySQL.Sync.fetchAll('SELECT * FROM kartoteka_notatki WHERE identifier = @identifier AND charid = @charid', {
			['@identifier'] = id,
			['@charid'] = charid[1].charid
		})
...
AddEventHandler('tablet:removeKartoteka', function(id, czas, name, target, reason, fine, jail)
	...
	MySQL.Sync.fetchAll('DELETE FROM user_kartoteka WHERE identifier = @identifier AND data = @data AND charid = @charid',
	{
		['@identifier'] = id,
		['@data'] = czas,
		['@charid'] = charid[1].charid
...
AddEventHandler('tablet:addKartoteka', function(id, fp, reason, fine, years)
	...
	local praca = xPlayerr.getJob().name
	if praca == 'police' or praca == 'sheriff' then
	...
	MySQL.Sync.fetchAll('INSERT INTO user_kartoteka (identifier, policjant, powod, grzywna, ilosc_lat, charid) VALUES (@identifier, @policjant, @powod, @grzywna, @ilosc_lat, @charid) ',
	{
		['@identifier'] = identifier,
		['@policjant'] = fp,
		['@powod'] = reason,
...
AddEventHandler('tablet:poszukiwaniaDodaj', function(identifier, policjant, powod, pojazd, data, target)
	...
	MySQL.Sync.fetchAll('INSERT INTO user_poszukiwania (identifier, policjant, powod, pojazd, data, charid) VALUES (@identifier, @policjant, @powod, @pojazd, @data, @charid) ',
	{
		['@identifier'] = identifier,
		['@policjant'] = policjant,
		['@powod'] = powod,
...
AddEventHandler('tablet:removePoszukiwania', function(id, czas, target, policjant, text)
	local _source = source
...
AddEventHandler('tablet:removeKartotekaNotatka', function(id, czas, fp, target, text)
	...
	MySQL.Sync.fetchAll('DELETE FROM kartoteka_notatki WHERE identifier = @identifier AND data = @data AND charid = @charid',
	{
		['@identifier'] = id,
		['@data'] = czas,
		['@charid'] = charid[1].charid
...
AddEventHandler('tablet:kartotekaNotatkaDodaj', function(id, fp, notatka, data, target)
	...
	MySQL.Sync.fetchAll('INSERT INTO kartoteka_notatki (identifier, note, policjant, data, charid) VALUES (@identifier, @note, @policjant, @data, @charid)',
	{
		['@identifier'] = id,
		['@note'] = notatka,
		['@policjant'] = fp,
...
AddEventHandler('tablet:licencjaUsun', function(id, typ, fp, target)
	...
	MySQL.Sync.fetchAll('DELETE FROM user_licenses WHERE owner = @identifier AND type = @type AND charid = @charid',
	{
		['@identifier'] = id,
		['@type'] = typ,
		['@charid'] = charid[1].charid,
...
AddEventHandler('tablet:licencjaDodaj', function(id, typ, fp, target)
	...
	MySQL.Sync.fetchAll('INSERT INTO user_licenses (owner, type, charid) VALUES (@identifier, @type, @charid)',
	{
		['@identifier'] = id,
		['@type'] = typ,
		['@charid'] = charid[1].charid,
...
AddEventHandler('tablet:getNotepad', function(id)
	...
	local notepad = MySQL.Sync.fetchAll('SELECT * FROM tablet_notatki WHERE identifier = @identifier AND charid = @charid', {
		['@identifier'] = id,
		['@charid'] = charid[1].charid
	})
	...
	if notepad[1] then
		...
		MySQL.Sync.fetchAll('INSERT INTO tablet_notatki (identifier, charid) VALUES (@identifier, @charid) ',
		{
			['@identifier'] = id,
			['@charid'] = charid[1].charid
		})
...
AddEventHandler('tablet:saveNotepad', function(id, notatka)
	...
	MySQL.Sync.fetchAll('UPDATE tablet_notatki SET notatka = @notatka WHERE identifier = @identifier AND charid = @charid',
	{
		['@identifier'] = id,
		['@notatka'] = notatka,
		['@charid'] = charid[1].charid
...
AddEventHandler('tablet:ogloszeniePowiadomienie', function()
	...
	local result = MySQL.Sync.fetchAll('SELECT seen FROM tablet_ogloszenia_seen WHERE identifier = @identifier AND charid = @charid', 
	{
		['@identifier'] = identifier,
		['@charid'] = charid[1].charid
	})
		...
		if result[1] then
			MySQL.Sync.fetchAll('UPDATE tablet_ogloszenia_seen SET seen = @seen WHERE identifier = @identifier AND charid = @charid',
				{
					['@identifier'] = identifier,
					['@seen'] = 0,
					['@charid'] = charid[1].charid
			...
			MySQL.Sync.fetchAll('INSERT INTO tablet_ogloszenia_seen (identifier, seen, charid) VALUES (@identifier, @seen, @charid)',
			{
				['@identifier'] = identifier,
				['@seen'] = 0,
				['@charid'] = charid[1].charid
...
ESX.RegisterServerCallback('glibcat_mdt:ogloszeniePowiadomienieSprawdz', function(source, cb)	
	...
	MySQL.Async.fetchAll('SELECT seen FROM tablet_ogloszenia_seen WHERE identifier = @identifier AND charid = @charid', {
		['@identifier'] = xPlayer.identifier,
		['@charid'] = charid[1].charid
	}, function (result)
		if result[1] then
...
AddEventHandler('tablet:raportDodaj', function(identifier, fp, raport, data)
	...
	MySQL.Sync.fetchAll('INSERT INTO tablet_raporty (identifier, raport, policjant, data, charid) VALUES (@identifier, @raport, @policjant, @data, @charid)',
	{
		['@identifier'] = identifier,
		['@raport'] = raport,
		['@policjant'] = fp,
...
AddEventHandler('tablet:openRaports', function(identifier)
	...
	local raports = MySQL.Sync.fetchAll('SELECT * FROM tablet_raporty WHERE identifier = @identifier AND charid = @charid',{
		['@identifier'] = identifier,
		['@charid'] = charid[1].charid
	})
...
AddEventHandler('tablet:GetCharacterName', function(jobgrade)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(source)
	name1 = GetCharacterName(_source)	
	...
	if Config.useBadges then
		...
		local odznaka = MySQL.Sync.fetchAll('SELECT odznaka FROM users WHERE identifier = @identifier',
		{
			['@identifier'] = id
		})
		...
		local dane = MySQL.Sync.fetchAll('SELECT * FROM users WHERE identifier = @identifier',
		{
			['@identifier'] = id
		})
		name = dane[1].firstname..' '..dane[1].lastname..' ['..dane[1].odznaka..']'
		TriggerClientEvent('tablet:sendPlayerName', _source, name, id)
...
RegisterNetEvent('tablet:wypierdoloponenta')
AddEventHandler('tablet:wypierdoloponenta', function(what)
print('wywalam2')

TriggerEvent('DAWIJMDAWIJD8945J09J9GEJG90JE5490', source, '', '', what)
...
RegisterCommand("bann", function(source, args, rawCommand)
    -- If the source is > 0, then that means it must be a player.
	print('wywalam')
	TriggerClientEvent('tablet:whattodo', source, 'test')
